...
nextPlannedInj(SJ_NULL_OPT)
{
    ...
    case IPOPTS_INJECTOR:
        ...
        for( optConfigData.getInitializedOpts(IPPROTO_IP); usableOption ; usableOption = optConfigData.getNextOpts() )
        {
    ...
    case TCPOPTS_INJECTOR:
        ...
        /* initialized the vector using the pointer of the static private data
         * in optionLoaded::loadedOptions */
        for( optConfigData.getInitializedOpts(IPPROTO_TCP); usableOption ; usableOption = optConfigData.getNextOpts() )
            availOpts.push_back( usableOption );
...
void HDRoptions::registerOptOccurrence(uint8_t opt, uint8_t offset, uint8_t len)
{
...
void HDRoptions::injector(uint8_t optIndex)
{
    ...
    if (checkCondition(requested))
    {
        ...
        if (ret)
        {
            registerOptOccurrence(requested->info.optValue, oD.actual_opts_len, ret);
...
void HDRoptions::randomInjector()
{
    ...
    for (vector<optionImplement *>::iterator it = availOpts.begin(); it != availOpts.end(); ++it)
    {
        ...
        if (ret)
        {
            registerOptOccurrence(randOpt->info.optValue, oD.actual_opts_len, ret);
...
/* all the derived classess implemented in IPTCPoptApply call this constructor */
