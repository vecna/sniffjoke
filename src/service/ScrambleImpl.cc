 ...
 *
 */
void TTLScramble::injectTTLProbe(TTLFocus &ttlfocus)
{
    Packet *injpkt;
    ...
    case TTL_BRUTEFORCE:
        ...
        else
        {
            ...
            injpkt->fixIPTCPSum();
//                p_queue.insert(*injpkt, SEND); // TODO XXX fixme il pkt va nel vector ?
 ...
 */
void TTLScramble::execTTLBruteforces(void)
{
 ...
 */
bool TTLScramble::extractTTLinfo(const Packet &incompkt)
{
...
bool TTLScramble::apply(Packet &pkt)
{
...
bool TTLScramble::mystification(Packet &pkt)
{
    TTLFocus &ttlfocus = ttlfocus_map->get(pkt);
    if (ttlfocus.status == TTL_KNOWN)
    {
...
bool TTLScramble::isScrambleAvailable(Packet &pkt)
{
...
bool TTLScramble::pktKeepRefresh(Packet &pkt)
{
    return (ttlfocus_map->get(pkt).status == TTL_BRUTEFORCE);
}

bool TTLScramble::periodicEvent(void)
{
    execTTLBruteforces();
    return false;
}

bool TTLScramble::scramInitSetup(void)
{
    LOG_DEBUG("Yay! TTL initalized, checking cache file, etc...");
    return true;
}

void TTLScramble::scramRegisterSession(Packet &triggering, SessionTrack &sex)
{
    LOG_DEBUG("start of this session: fixme: this value has to be called in the Session get/find/costructor");
}

TTLScramble::TTLScramble(vector<Packet *> *SingletonScrambleV) :
ScrambleImpl::ScrambleImpl(TTL, "TTL", SingletonScrambleV, false, BEFORE_CHECKSUM)
{
}

TTLScramble::~TTLScramble(void)
{
    LOG_VERBOSE("");
}

bool CKSUMScramble::apply(Packet &pkt)
{
    switch(pkt.proto)
    {
        case OTHER_IP:
...
bool CKSUMScramble::mystification(Packet &pkt)
{
    return false;
}

bool CKSUMScramble::isScrambleAvailable(Packet &pkt)
{
...
bool CKSUMScramble::pktKeepRefresh(Packet &pkt)
{
    return true;
}

bool CKSUMScramble::periodicEvent(void)
{
    return false;
}

bool CKSUMScramble::scramInitSetup(void)
{
    LOG_DEBUG("Yay! check initalized, but has nothing to be init ");
    return true;
}

void CKSUMScramble::scramRegisterSession(Packet &triggering, SessionTrack &sex)
{
    LOG_DEBUG("we don't care, I belive, about registering checksummed unlucky38 sessions");
}

CKSUMScramble::CKSUMScramble(vector<Packet *> *SingletonScrambleV) :
...
CKSUMScramble::~CKSUMScramble(void)
{
    LOG_VERBOSE("");
}
