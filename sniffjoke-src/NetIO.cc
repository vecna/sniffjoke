    ...
    while( burst-- )
    {
        ...
        if(fds[0].revents & POLLIN)
        {
            if((nbyte = recv(netfd, pktbuf, MTU, 0)) == -1)
            {
                ...
                if( conntrack->add_packet_queue(NETWORK, pktbuf, nbyte))
                    io_happened = true;
            }
        }
        ...
        if(fds[1].revents & POLLIN)
        {
            if((nbyte = read(tunfd, pktbuf, MTU)) == -1)
            {
                ...
                /* add packet in connection tracking queue */
                if( conntrack->add_packet_queue(TUNNEL, pktbuf, nbyte))
                    io_happened = true;
            }
        }
    }
...
void NetIO::queue_flush()
{
     ...
     */
    packet = conntrack->p_queue->get(SEND, ANY_SOURCE, ANY_PROTO, false);
    while( packet != NULL )
    {
        ...
        conntrack->p_queue->drop(packet);
        packet = conntrack->p_queue->get(SEND, ANY_SOURCE, ANY_PROTO, true);
    }
}
