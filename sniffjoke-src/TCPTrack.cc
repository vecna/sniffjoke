...
void TCPTrack::manage_outgoing_packets( Packet *pkt )
{
	...
	pkt->status = SEND;
	ct = sex_list->get(pkt);
	if( ct != NULL && (pkt->tcp->rst || pkt->tcp->fin ))
	{
#ifdef PACKETDEBUG
		internal_log(NULL, PACKETS_DEBUG,
 ...
 */
void TCPTrack::enque_ttl_probe( Packet *delayed_syn_pkt, TTLFocus* ttlfocus)
{
...
void TCPTrack::mark_real_syn_packets_SEND(unsigned int daddr) {
	...
	Packet *packet = packet = p_queue->get(ANY_STATUS, ANY_SOURCE, TCP, false);
	while( packet != NULL )
	{
...
void TCPTrack::SjH__inject_ipopt( Packet *hackpkt )
{
	...
	int max_route_n = (hackpkt->pbuf_size - ntohs(hackpkt->ip->tot_len)) / 4;
