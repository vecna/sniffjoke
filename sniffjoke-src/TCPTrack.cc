...
bool TCPTrack::add_packet_queue( const source_t source, const unsigned char *buff, int nbyte )
{
    Packet *pkt;
...
Packet* TCPTrack::analyze_incoming_icmp( Packet *timeexc )
{
    ...
    tf = ttlfocus_list->get(badiph->daddr);
    if(tf == NULL) {
        tf = new TTLFocus(badiph->daddr);
        ttlfocus_list->insert(tf);
    }
...
Packet* TCPTrack::analyze_incoming_synack( Packet *synack )
{
     ...
     */
    if((tf = ttlfocus_list->get(synack->ip->saddr)) == NULL) 
        return synack;
...
Packet* TCPTrack::analyze_incoming_rstfin( Packet *rstfin )
{
    ...
    ct = sex_list->get(rstfin->ip->saddr, rstfin->tcp->dest, rstfin->tcp->source);
    if(ct != NULL) {
        /* 
 ...
 *
 */
void TCPTrack::inject_hack_in_queue( const Packet *pkt, SessionTrack *ct ) 
{
    Packet *injpkt;
    ...
    int hpool_len = 0;
    int payload_len = ntohs(pkt->ip->tot_len) - ((pkt->ip->ihl * 4) + (pkt->tcp->doff * 4));
...
sendchosenhacks:
    ...
    /* -- FINALLY, SEN THE CHOOSEN PACKET(S) */
    if(hpool_len)
    {
        judge_t court_word;
        ...
        for(int i = 0; i < hpool_len; i++) 
        {
            ...
            internal_log(NULL, HACKS_DEBUG,
                ...
                ntohs(injpkt->tcp->dest), ct->packet_number,
                ntohs(injpkt->ip->id),
 ...
 */
void TCPTrack::enque_ttl_probe( Packet *delayed_syn_pkt, SessionTrack *ct )
{
    ...
    injpkt->tcp->source = ct->tf->puppet_port;
    injpkt->tcp->seq = htonl(ct->tf->rand_key + tested_ttl);
    injpkt->ip->id = (ct->tf->rand_key % 64) + tested_ttl;
...
bool TCPTrack::analyze_ttl_stats( SessionTrack *ct )
{
...
void TCPTrack::mark_real_syn_packets_SEND(unsigned int daddr) {
    ...
    packet = p_queue->get(ANY_STATUS, ANY_SOURCE, TCP, false);
    while( packet != NULL )
    {
...
Packet* TCPTrack::packet_orphanotrophy( const Packet* pb, int resize )
{
    Packet *ret = new Packet(pb);
    ret->proto = TCP;
    ret->source = LOCAL;
    ret->status = SEND;
...
SessionTrack * TCPTrack::init_session( const Packet *pkt ) 
{
    /* pkt is the refsyn, SYN packet reference for starting ttl bruteforce */
    SessionTrack *session = new SessionTrack(pkt);
    sex_list->insert(session);
    session->tf = ttlfocus_list->get(pkt->ip->daddr);
    if(session->tf == NULL) {
        session->tf = new TTLFocus(pkt->ip->daddr);
        ttlfocus_list->insert(session->tf);
    }

#ifdef PACKETDEBUG
    internal_log(NULL, PACKETS_DEBUG,
            "Session: local:%d -> %s:%d (ISN %08x) puppet %d TTL exp %d wrk %d", 
            ntohs(session->sport), 
            inet_ntoa( *((struct in_addr *)&session->daddr) ) ,
            ntohs(session->dport),
            session->isn,
            ntohs(session->tf->puppet_port),
            session->tf->expiring_ttl,
            session->tf->min_working_ttl
    );
#endif
...
void TCPTrack::SjH__fake_data( Packet *hackpkt )
{
    ...
    diff = ntohs(hackpkt->ip->tot_len) - ( (hackpkt->ip->ihl * 4) + (hackpkt->tcp->doff * 4) );
...
{
    int original_size = hackpkt->orig_pktlen - (hackpkt->ip->ihl * 4) - (hackpkt->tcp->doff * 4);
    hackpkt->ip->id = htons(ntohs(hackpkt->ip->id) + (random() % 10));
    ...
    if(1) //if(random() % 2)
        ...
        hackpkt->tcp->rst = 1; /* FIXME, a fake rst seems to break connection */
...
void TCPTrack::SjH__inject_ipopt( Packet *hackpkt )
{
    ...
    int l47len = ntohs(hackpkt->ip->tot_len) - iphlen;
    int max_route_n = ((MTU - 8) - ntohs(hackpkt->ip->tot_len)) / 4;
    ...
    int startipopt = hackpkt->ip->ihl * 4 - sizeof(struct iphdr);
    ...
    hackpkt->pbuf_size = iphlen + l47len;
...
void TCPTrack::SjH__inject_tcpopt( Packet *hackpkt ) 
{
    int iphlen = hackpkt->ip->ihl * 4;
    int tcphlen = hackpkt->tcp->doff * 4;
    int l57len = ntohs(hackpkt->ip->tot_len) - ( iphlen + tcphlen );
    int faketcpopt = 8;
    ...
    hackpkt->pbuf_size = iphlen + tcphlen + faketcpopt + l57len;
}
