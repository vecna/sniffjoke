...
Packet::Packet(const Packet* pkt) {
    ...
    pbuf_size = orig_pktlen = pkt->pbuf_size;
...
void Packet::resizePayload(int newpayloadlen) {
    int iphlen = ip->ihl * 4;
    int tcphlen = tcp->doff * 4;
    int payloadlen = ntohs(ip->tot_len) - ( iphlen + tcphlen );
    int newpbuf_size = pbuf_size - payloadlen + newpayloadlen;
    unsigned char *newpbuf = (unsigned char *)calloc(1, newpbuf_size);
    int newtotallen = iphlen + tcphlen + newpayloadlen;
